...
| 2 | **东财 push2delay 镜像 → push2 → akshare** | 行业板块排名 + **主力净流入(f62)** | ✅ 沙盒/生产双通 |
...
- **腾讯指数API vals[37] 成交额单位=万**：万亿格式需除100000000（万→万亿），亿格式需除10000（万→亿）。之前除错导致"16211.96万亿"（实际1.62万亿）
- **腾讯指数API vals[48]/[49] 非交易日返回-1**：需过滤<0的值，不显示时只保留振幅信息
- **周末日期自动回退**：周六→周五、周日→周五，确保报告日期匹配实际交易日数据
- **后排/跟风不能靠强势股涨幅分层**：同花顺强势股涨幅普遍>3%，几乎没有落入后排的。改为用涨幅较低的行业板块（第4-6名净流入行业）填充后排
